...
def send_pending_products_for_moderation(call):
    ...
    for product in pending_products:
        ...
        try:
            if photos:
                media_group = []
                media_group.append(types.InputMediaPhoto(photos[0], caption=admin_message_text, parse_mode='Markdown'))
                for photo_id in photos[1:]:
                    media_group.append(types.InputMediaPhoto(photo_id))
                
                sent_messages = bot.send_media_group(call.message.chat.id, media_group)
                # Оскільки кнопки не можна додати до медіа-групи, відправляємо їх окремим повідомленням,
                # відповідаючи на перше повідомлення медіа-групи.
                bot.send_message(call.message.chat.id, "Оберіть дію:", reply_markup=markup_admin,
...
def send_admin_faq_menu(call):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🔙 Назад до Адмін-панелі", callback_data="admin_panel_main")
    )
...
def handle_admin_faq_callbacks(call):
    ...
    if action == "add":
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Назад до FAQ меню", callback_data="admin_faq_menu"))
...
if __name__ == '__main__':
    ...
    logger.info(f"Вебхук встановлено на: {WEBHOOK_URL_BASE + WEBHOOK_URL_PATH}")

    # Запускаємо Flask додаток
    # Heroku надає порт через змінну середовища PORT
    port = int(os.environ.get("PORT", 5000))
    logger.info(f"Бот запускається на порту {port}...")
