...
def process_product_input(message):
    ...
    if current_step == 'waiting_name':
        ...
        if len(message.text) > 50:
            bot.send_message(chat_id, "❌ Ціна занадто довга. Будь ласка, введіть ціну до 50 символів.")
            return
        ...
        bot.send_message(chat_id,
                         "📝 *Крок 3/4: Опис*\n\n"
                         "Введіть детальний опис товару (до 1000 символів):",
                         parse_mode='Markdown')

    elif current_step == 'waiting_description':
        if not (10 <= len(message.text) <= 1000):
            bot.send_message(chat_id, "❌ Опис товару повинен бути від 10 до 1000 символів.")
            return
        ...
        bot.send_message(chat_id,
                         "📸 *Крок 4/4: Фото*\n\n"
                         "Надішліть фотографію товару. Це буде останній крок перед модерацією. "
                         "Будь ласка, надсилайте фото як документ, щоб зберегти якість, або як стиснуте фото, але високої якості.",
                         parse_mode='Markdown')

    # Якщо користувач ввів текст, але він не відповідає очікуваному кроку
    else:
        # Цей блок спрацює, якщо current_step не 'waiting_name', 'waiting_price', 'waiting_description', 'waiting_photo'
        # і користувач все ще в user_data, але відправив текст, який не є відповіддю на очікуване запитання.
        # Можемо просто ігнорувати або повідомити користувача.
        bot.send_message(chat_id, "Неочікуване текстове повідомлення. Будь ласка, дотримуйтесь процесу додавання товару.")
        # Можна також скинути стан, якщо це бажано:
        # if chat_id in user_data:
        #     del user_data[chat_id]
        #     bot.send_message(chat_id, "Процес додавання товару було скинуто. Спробуйте почати з /add_product знову.")
...
def process_photo(message):
    ...
    if chat_id not in user_data or user_data[chat_id]['step'] != 'waiting_photo':
        return # Ігноруємо фото, якщо воно не очікується

    # Отримуємо file_id найбільшої фотографії
    photo_file_id = message.photo[-1].file_id
    user_data[chat_id]['photo'] = photo_file_id
    ...
    product_id = add_product(
        ...
        user_data[chat_id]['price'],
        user_data[chat_id]['photo']
    )

    # Очищаємо дані користувача після збереження товару
    del user_data[chat_id]

    bot.send_message(chat_id,
                     "✅ Ваш товар успішно відправлено на модерацію. "
