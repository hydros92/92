...
# Gemini AI API налаштування
GEMINI_API_KEY = os.getenv('GEMINI_API_KEY', '') # Ваш ключ Gemini API
...
# --- 7. Функції роботи з користувачами та загальні допоміжні функції ---
@error_handler
...
def get_gemini_response(prompt, conversation_history=None):
    ...
    try:
        api_url = f"{GEMINI_API_URL}?key={GEMINI_API_KEY}" if GEMINI_API_KEY else GEMINI_API_URL
        ...
        logger.error(f"Помилка HTTP запиту до Gemini API: {e}")
        return generate_elon_style_response(prompt)
    except Exception as e:
        logger.error(f"Загальна помилка при отриманні відповіді від Gemini: {e}")
        return generate_elon_style_response(prompt)
...
def start_add_product_flow(message):
    ...
    chat_id = message.chat.id
    # Ліміт на кількість товарів на модерації знято, тому код нижче закоментовано.
    # conn = get_db_connection()
    # cursor = conn.cursor()
    # try:
...
# --- 12. Обробники текстових повідомлень та кнопок меню ---
...
def send_my_products(message):
    ...
    if user_products:
        ...
        for i, product in enumerate(user_products, 1):
            ...
            response += f"   📅 {datetime.strptime(product['created_at'], '%Y-%m-%d %H:%M:%S').strftime('%d.%m.%Y %H:%M')}\n"
            response += f"   📊 Статус: {status_ukr}\n"
            
            if product['status'] == 'approved' and product['channel_message_id']:
                # Динамічно формуємо посилання на канал за його ID, оскільки username може бути відсутнім
                # Посилання формату t.me/c/channel_id/message_id
                channel_link_part = str(CHANNEL_ID).replace("-100", "") # Видаляємо "-100" з початку ID каналу для посилання
...
def send_help_message(message):
    ...
    help_text = (
        ...
        f"Якщо виникли технічні проблеми, зверніться до адміністратора: @{'Король Хрести' if ADMIN_CHAT_ID else 'не встановлено'}" # TODO: Замініть на свій username
    )
...
def send_admin_statistics(call):
    ...
    try:
        ...
        blocked_users_count = cursor.fetchone()['COUNT(*)']
...
def send_pending_products_for_moderation(call):
    ...
    for product in pending_products:
        ...
        admin_message_text = (
            ...
            f"📅 *Додано:* {datetime.strptime(product['created_at'], '%Y-%m-%d %H:%M:%S').strftime('%d.%m.%Y %H:%M')}"
        )
        ...
        try:
            ...
            logger.error(f"Помилка при відправці товару {product_id} на модерацію адміністратору: {e}")
...
# --- 17. Callbacks для модерації товару ---
@error_handler
...
# --- 18. Повернення до адмін-панелі після колбеку ---
...
if __name__ == '__main__':
    ...
    # Отримуємо URL вебхука зі змінних середовища
    WEBHOOK_URL = os.getenv("WEBHOOK_URL")
    if not WEBHOOK_URL:
        logger.warning("WEBHOOK_URL не встановлено в .env. Запускаємо бота в режимі polling.")
        try:
            bot.infinity_polling(logger_level=logging.DEBUG, skip_pending=True)
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Вебхук встановлено: {WEBHOOK_URL}/{TOKEN}")
            # Запускаємо Flask додаток
            # Використовуйте '0.0.0.0' для прослуховування всіх публічних інтерфейсів
            # Порт береться зі змінних середовища або за замовчуванням 5000
            app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 5000)))
        except Exception as e:
            logger.critical(f"Критична помилка при запуску webhook або Flask: {e}", exc_info=True)
